...
export default function ReportTable({
    ...
    const { user } = useAuth()
    // เห็นเฉพาะแผนกตัวเอง ยกเว้นกลุ่ม CAD/CAM (d_id 1,2,3) ที่เห็นกันเองในกลุ่ม — ไม่มีข้อยกเว้นสำหรับ admin/subadmin
    const visibleDepartmentIds = useMemo(() => getVisibleDepartmentIds(user?.d_id), [user?.d_id])
    ...
    const enrichedData = useMemo<EnrichedRow[]>(() => {
        ...
        return data.reduce<EnrichedRow[]>((rows, row) => {
            const d_id = usercodeToDepartment.get(row.e_usercode)
            if (d_id == null || !visible.has(d_id)) return rows
            rows.push({ ...row, department_id: String(d_id) })
            return rows
        }, [])
    }, [data, usercodeToDepartment, visibleDepartmentIds])
    ...
    const departmentOptions = useMemo(() => {
        ...
        return departments
            .filter((department) => visibleDepartmentIds.includes(department.d_id))
            .map((department) => ({
    ...
    }, [enrichedData, selected, departments, visibleDepartmentIds])
