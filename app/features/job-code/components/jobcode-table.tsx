...
export default function JobCodeTable({
    data,
    loading,
    error,
    departments,
    ...
    const persisted = usePersistedTanstackTable("job_code", {
        ...
        defaultSorting: [{ id: "job_id", desc: false }],
    })
    ...
    const table = useReactTable({
        ...
        initialState: { columnVisibility: { job_id: false } },
    ...
    return (
        <Card>
            <CardHeader>
                <p className="font-medium">รายการรหัสงาน</p>
            </CardHeader>
            <CardContent>
                <div className="space-y-4 md:w-full">
                    <div className="grid w-full gap-3 md:mr-4 md:grid-cols-4">
                        <Input
                            ...
                            value={(table.getColumn("job_code")?.getFilterValue() as string) ?? ""}
                            onChange={(event) =>
                                table.getColumn("job_code")?.setFilterValue(event.target.value)
                            }
                            className="w-full md:w-auto"
                        />
                        <Input
                            placeholder="ค้นหารายละเอียด..."
                            value={(table.getColumn("job_descriptions")?.getFilterValue() as string) ?? ""}
                            onChange={(event) =>
                                table.getColumn("job_descriptions")?.setFilterValue(event.target.value)
                            }
                            className="w-full md:w-auto"
                        />
                        <Combobox
