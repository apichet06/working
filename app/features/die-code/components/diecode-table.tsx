...
import { getDieCodeColumns } from "./columns"
import { Department, DieCode } from "../type"
...
export default function DieCodeTable({
    data,
    loading,
    error,
    departments,
    ...
    const columns = useMemo(
        () => getDieCodeColumns({ onEdit, onDelete }),
        [onEdit, onDelete]
    )
    ...
    const persisted = usePersistedTanstackTable("die_code", {
        defaultPagination: { pageIndex: 0, pageSize: 5 },
        defaultSorting: [{ id: "die_id", desc: false }],
    })
    ...
    const table = useReactTable({
        ...
        initialState: { columnVisibility: { die_id: false } },
    ...
    return (
        <Card>
            <CardHeader>
                <p className="font-medium">รายการรหัสดาย</p>
            </CardHeader>
            <CardContent>
                <div className="space-y-4 md:w-full">
                    <div className="grid w-full gap-3 md:mr-4 md:grid-cols-4">
                        <Input
                            ...
                            value={(table.getColumn("die_code")?.getFilterValue() as string) ?? ""}
                            onChange={(event) =>
                                table.getColumn("die_code")?.setFilterValue(event.target.value)
                            }
                            className="w-full md:w-auto"
                        />
                        <Input
                            placeholder="ค้นหารายละเอียด..."
                            value={(table.getColumn("die_descriptions")?.getFilterValue() as string) ?? ""}
                            onChange={(event) =>
                                table.getColumn("die_descriptions")?.setFilterValue(event.target.value)
                            }
                            className="w-full md:w-auto"
                        />
                        <Combobox
